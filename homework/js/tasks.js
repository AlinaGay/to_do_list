...
function buildCalendar() {
  ...
  var dayCounter = 1;
  var prevMonthLastDay = previousMonthLastDay();
  var str_out_week = '';
  while(j < 6) {
    var str_out = '';
    for(var i = 1; i < 8; i++, k++) {
      if (firstDay.dayWeek > i && j == 1) {
          //str_out += '<td>&nbsp;</td>';
          var previousMonthDay = prevMonthLastDay - (firstDay.dayWeek - i - 1);
          str_out += drawCell(' class="otherMonth"', i, previousMonthDay);
      } else if (dayCounter > firstDay.maxDays ) {
        var nextMonthDay = dayCounter - firstDay.maxDays;
          str_out += drawCell(' class="otherMonth"', i, nextMonthDay);
          dayCounter++;
      } else {
        var todayClass = '';
        if ( k == dayMonth ) { //
        ...
        str_out += drawCell(todayClass, i, dayCounter);
          dayCounter++;
      }
...
function drawCell(todayClass, weekDayNumber, dayCounter) {
      return '<td' + todayClass + ' data-weekday="'+ weekDayNumber +'" data-daymonth="' + dayCounter + '">' + dayCounter + '</td>';
}
...
function previousMonthLastDay() {
    var date = new Date();
    var dayMonth = date.getDate();
    date.setDate(date.getDate()-dayMonth-1);
    return date.getDate();
}
